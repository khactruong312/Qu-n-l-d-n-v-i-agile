            ...
            <div class="comment-item" data-id="<?php echo $comment_id ?>">
                <div class=" flex items-start space-x-3 ">
                    <img class="w-10 h-10 rounded-full"
                        src="<?php echo !empty($user['image_url']) ? './upload/' . $user['image_url'] : "assets/img/mock-avatar.svg" ?>"
                        alt="">
                            ...
                            foreach ($children as $item) {
                                ...
                                <div class="flex items-start space-x-3 mt-4">
                                    <img class="w-10 h-10 rounded-full"
                                        ...
                                        alt="">
                                    <div
                                        class="flex flex-col bg-violet-100 bg-opacity-50 rounded-3xl px-3 py-4 min-w-[400px] max-w-[80%] relative">
                                        <p class="mb-0 text-xs font-semibold"><?php echo $user['name'] ?></p>
                        ...
                        <div>
                            <div class="flex items-start space-x-4">
                                <img class=" w-8 h-8 rounded-full"
                                    src="<?php echo !empty($_SESSION['users']['image_url']) ? './upload/' . $_SESSION['users']['image_url'] : "assets/img/mock-avatar.svg" ?>"
                                    alt="">
                                <div class="shrink min-w-[400px] max-w-[80%] relative">
                                    <textarea name="content_<?php echo $comment_id ?>"
                                        class="comment-content form-textarea w-full rounded-md px-2 py-3 border-0 outline-none ring-0 bg-violet-50 bg-opacity-25 focus:bg-white"
                                        rows="2" placeholder="Reply this comment"></textarea>
                                    <button type="button"
                                        class="send-comment btn btn-circle btn-sm absolute right-3 bottom-4 btn-disabled">
